...
int MesosContainerizerLaunch::execute()
{
  ...
  if (flags.pre_exec_commands.isSome()) {
    ...
    foreach (const JSON::Value& value, array.values) {
      ...
      if (parse->shell()) {
        cout << "Executing pre-exec command " << vlue << endl;
      ...
      if (s.isError()) {
        cerr << "Failed to create subprocess: " << s.error() << endl;
        return EXIT_FAILURE;
      }

      s->status().await();
      if (!s->status().isReady()) {
        cerr << "Failed to finish subprocess: "
      ...
      if (status.isNone()) {
        cerr << "Failed to reap the subprocess" << endl;
        return EXIT_FAILURE;
      } else if (status.get() != 0) {
        cerr << "Failed to execute '"
             << parse->value() << "' as a subprocess"
             << endl;
        return EXIT_FAILURE;
      }
    }
