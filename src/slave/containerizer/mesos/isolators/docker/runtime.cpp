    ...
    const ContainerConfig& containerConfig)
{
  ...
  // command executor as flags. For custom executor and default
  // executor cases, these information will be included in
  // 'ContainerLaunchInfo', and will be passed back to containerizer.
  if (containerConfig.has_task_info()) {
    ...
    // The custom executor and default executor cases.
    if (workingDirectory.isSome()) {
    ...
    const ContainerConfig& containerConfig)
{
  ...
  // on the logic table below. For custom executors and the default
  // executor cases, we make changes to the command directly. For
  // command task case, if no need to change the launch command for
  // the user task, we do not do anything and return the CommandInfo
  // from ContainerConfig. We only add a flag `--task_command` to
  // carry command as a JSON object if it is necessary to mutate.
  CommandInfo command;
  ...
  if (containerConfig.has_task_info()) {
    ...
    // Custom executor or the default executor case.
