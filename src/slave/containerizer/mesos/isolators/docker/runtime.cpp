    ...
    const Option<string>& workingDir)
{
  ...
  // on the logic table below. For custom executor case, we make changes
  // to the command directly. For command task case, if no need to
  // change the launch command for the user task, we do not do anything
  // and return the CommandInfo from ExecutorInfo. We only add a flag
  // `--task_command` to carry command as a JSON object if it is
  // neccessary to mutate.
  CommandInfo command;
  ...
  if (command.shell()) {
    ...
    if (!command.has_value()) {
      ...
      // a value, so that arguments can be specified by user, which
      // is running with default image Entrypoint.
