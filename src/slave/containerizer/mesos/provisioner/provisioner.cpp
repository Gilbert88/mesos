...
{
  // List provisioned containers, recover living ones, and destroy
  // unknown containers. Note that known orphan containers are
  // recovered as well and they will be destroyed by the containerizer
  // using the normal cleanup path. See MESOS-2367 for details.
  //
  // NOTE: For the nested container case, all containers (in any
  // hierarchy) will be included in one hashset.
  ...
  // but mark unknown containers for immediate cleanup. For nested
  // containers case, this hashset should still include all unknown
  // containers from all hierarchy.
  ...
  foreach (const ContainerID& containerId, unknownContainerIds) {
    ...
    // If a container is unknown, it means the launcher
    // has not forked it yet. So an unknown container
    // should not have any child. It means that when
    // destroying an unknown container, we can just simply
    // call 'destroy' directly, without needing to make
    // a recursive call to destroy. However, every time
    // we destroy a container in provisioner, we should
    // still check that no nested container exists.
  ...
  // in 'store', which might fail if there still exist unknown containers
