...
public:
  ...
  Future<ImageSpecs> get(const Image& image);
...
Future<ImageSpecs> Store::get(const Image& image)
{
...
Future<ImageSpecs> StoreProcess::get(const Image& image)
{
  ...
  foreach (const CachedImage& candidate, images[appc.name()].values()) {
    ...
    if (matches(appc, candidate)) {
      ...
      // We changed the interface of store get to pass runtime
      // configurations to mesos containerizer.
      ...
      return ImageSpecs{vector<string>({candidate.rootfs()}),
                        None()};
    }
  }
