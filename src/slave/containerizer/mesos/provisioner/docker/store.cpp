...
public:
  ...
  Future<ImageSpecs> get(const mesos::Image& image);
...
private:
  ...
  Future<ImageSpecs> __get(const Image& image);
...
Future<ImageSpecs> Store::get(const mesos::Image& image)
{
  return dispatch(process.get(), &StoreProcess::get, image);
}


Future<ImageSpecs> StoreProcess::get(const mesos::Image& image)
{
...
Future<ImageSpecs> StoreProcess::__get(const Image& image)
{
  ...
  // TODO(gilbert): We should simply do protobuf::parse here
  // to convert all pre-structured JSON object as RuntimeConfig
  // protobuf.
  Option<RuntimeConfig> runtimeConfig = None();

  return ImageSpecs{layerDirectories, runtimeConfig};
}
