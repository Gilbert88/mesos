    ...
    const Option<NvidiaComponents>& nvidia)
{
  ...
  const hashmap<string, lambda::function<Try<Isolator*>(const Flags&)>>
    ...
    {"volume/image", &VolumeImageIsolatorProcess::create},
...
{
  Owned<Provisioner> _provisioner = provisioner;

  process = Owned<MesosContainerizerProcess>(new MesosContainerizerProcess(
      flags,
      local,
      fetcher,
      logger,
    ...
    bool checkpoint)
{
  ...
  //  3) A custom executor or a command task with ContainerInfo,
  //     but no image is specified.
  // In either of the above cases, no provisioning is needed.
  // Therefore, we can go straight to 'prepare'.
  if (!executorInfo.has_container() ||
      containerImage.isNone()) {
  ...
  return provisioning
    .then(defer([=](const ProvisionInfo& provisionInfo) -> Future<bool> {
