...
// This test verifies that the missing system configuration files are
// mounted from the host into the container correctly.
TEST_F(DockerRuntimeIsolatorTest, ROOT_INTERNET_CURL_MountSystemConfiguration)
{
  Try<Owned<cluster::Master>> master = StartMaster();
  ASSERT_SOME(master);
  ...
  // The system config file /etc/resolv.conf does not exist in
  // alpine's rootfs, so we use it to test mounting system config.
  // We expect TASK_FINISHED if this file can be successfully
  // mounted from the host into the container.
  CommandInfo command;
  command.set_shell(false);
  command.set_value("/bin/cat");
  command.add_arguments("cat");
  command.add_arguments("/etc/resolv.conf");
